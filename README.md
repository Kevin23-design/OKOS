...
## 实验反思
在本次实验中，通过实现 uvm 的 copyin/copyout/字符串拷贝、syscall 分发、堆的 brk 管理、栈的按需扩展和 mmap 节点池，深入理解了用户态与内核态地址转换、页表映射与缺页处理的实际机制；对多级页表、PTE 权限和物理页分配有了直观认识。
同时，实现页表深拷贝与销毁的完整测试（为 fork 做准备）、在多核并发场景下验证 mmap 仓库的正确性、加入更完善的内存使用统计与泄漏检测工具。